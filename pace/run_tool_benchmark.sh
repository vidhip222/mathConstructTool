#!/usr/bin/env bash
# =============================================================================
# pace/run_tool_benchmark.sh — SLURM job: MathConstruct benchmark (tool solver)
# =============================================================================
# Submit AFTER the vLLM server job is running:
#
#   SERVER_JOB=$(sbatch --parsable pace/server_llama70b.sh)
#   sbatch --dependency=after:${SERVER_JOB} pace/run_tool_benchmark.sh
#
# Adjust --account to match your PACE allocation.
# =============================================================================

#SBATCH --job-name=mc-tools
...
#SBATCH --output=logs/tools_%j.out
#SBATCH --error=logs/tools_%j.err
...
echo "Starting tool-calling benchmark ..."
uv run python src/scripts/run.py "${TEMP_CONFIG}"

echo "Done. Results in outputs/llama-tools-run/"
rm -f "${TEMP_CONFIG}"
